...
app.get('/ta_error', function (req, res, next) {
...
describe('Test error of correct format - production', function () {
  ...
  it('trivechain error', function (done) {
    request(app)
			.get('/ta_error')
			.expect(400)
			.end(function (err, res) {
	...
	it('trivechain error, with explanation', function (done) {
    request(app)
			.get('/ta_error?explain=true')
			.expect(400)
			.end(function (err, res) {
				if (err) return done(err)
				assert.deepEqual(res.body, {
...
describe('Test error of correct format - development', function () {
  ...
  it('trivechain error with request-id, correlation-id and remote-id', function (done) {
  ...
  it('requestId is carried from the error-initiator', function (done) {
  	this.timeout(5000)
		var requestIdMiddleware = require('ta-request-id')
